...
<?php 
     require_once("connexio.php");
     if(!empty($_GET['code'])){
        $req= $bdd->prepare("SELECT * FROM activite WHERE cod=?");
   ...
   <div class="connexion">  <?php echo "Bonjours ".$_SESSION['NOM']['nom'] ?>  </div>
    <section class="tete">
        ...
        <div class="formulaire1">
            <form action="modifiactivite.php" method="POST" enctype="multipart/form-data" class="formulaire2" >
               <label class="formulaire3" for="">CODE</label>
                <input type="text" name="logo" class="formule3" value="<?php echo ($modif['cod']) ?>">
                ...
                <input type="text" name="logo2" id="" class="formule3" value="<?php echo ($modif['descriptionac']) ?>">
                <label class="formulaire3"  for="">IMAGE</label> <br>
                <input type="file" name="logo3" class="formule3" >
                <img src="images/<?php echo ($modif['imageac']) ?>" alt="" width="60" height="60">
                <label class="formulaire3"  for="">TEXTE</label> <br>
                <input type="text" name="logo4" class="formule3" value="<?php echo ($modif['texte']) ?>">
...
<?php 
   require_once("connexio.php");
    ...
    if (isset($_POST['logo5'])) {
        ...
        //$cod =$_POST['logo'];
       if($photo1 === ""){
          $req= $bdd->prepare("UPDATE activite SET titre=?, descriptionac=?, texte=?  WHERE cod=?");
          ...
          header("location:afficheactivite.php");
            ...
            $req= $bdd->prepare("UPDATE activite SET titre=?, descriptionac=?, imageac=?, texte=?  WHERE cod=?");
