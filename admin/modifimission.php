...
<?php 
     require_once("connexio.php");
     if(!empty($_GET['code'])){
        $req= $bdd->prepare("SELECT * FROM mission WHERE codem=?");
   ...
   <div class="connexion">  <?php echo "Bonjours ".$_SESSION['NOM']['nom'] ?>  </div>
    <section class="tete">
        ...
        <div class="formulaire1">
            <form action="modifimission.php" method="POST" enctype="multipart/form-data" class="formulaire2" >
               <label class="formulaire3" for="">CODE </label>
                <input type="text" name="logo" class="formule3" value="<?php echo ($modif['codem']) ?>">
                <label class="formulaire3" for="">TITRE </label> <br>
                <input type="text" name="logo1" class="formule3" value="<?php echo ($modif['titrem']) ?>">
                <label class="formulaire3"  for="">DESCRIPTION</label> <br>
                <input type="text" name="logo2" id="" class="formule3" value="<?php echo ($modif['descriptionm']) ?>">
                <label class="formulaire3"  for="">TEXTE</label> <br>
                <textarea name="logo3" id="" class="formule3"> <?php echo ($modif['textem']) ?> </textarea>
                <label class="formulaire3"  for=""> IMAGE</label> <br>
                <input type="file" name="pho" class="formule3" >
                <img src="images/<?php echo ($modif['imagem']) ?>" alt="" width="60" height="60">
...
<?php 
   require_once("connexio.php");
    ...
    if (isset($_POST['logo5'])) {
       if($_FILES['pho']['name']===""){
            $req= $bdd->prepare("UPDATE mission SET titrem=?, descriptionm=?, textem=? WHERE codem=?");
            ...
            $req= $bdd->prepare("UPDATE mission SET titrem=?, descriptionm=?, textem=?, imagem=? WHERE codem=?");
