...
<?php  
   if(!empty($_GET['code'])){
		$supp=$bdd->prepare("DELETE FROM projet WHERE codepr=?");    
...
<h1> <a href="../index.php">ACCUEIL</a></h1>
 ...
 <div class="formulai">
<table>
<h3 class="lien"> <a href="projet.php">ajouter</a></h3>
  <tr class="tab">
      <td>code</td>
     <td>TITRE</td>
     <td>DESCRIPTION</td>
	...
	<?php $reponse=$bdd->query("SELECT * FROM projet"); ?>
  	<?php while ($donne=$reponse->fetch()) { ?> 		
  <tr>
        <td> <?php echo ($donne['codepr']) ?> </td>
        <td> <?php echo ($donne['titrepr']) ?> </td>
    	<td> <?php echo ($donne['descriptionpr']) ?> </td>
        <td> <img src="images/<?php echo ($donne['imagefontpr']) ?>" alt="" width="60" height="60">  </td>
        <td> <?php echo ($donne['textepr']) ?> </td>                           
        <td> <a href="modifiprojet.php?code=<?php echo ($donne['codepr'])?>">modifier</a> </td>
        <td> <a href="afficheprojet.php?code=<?php echo ($donne['codepr'])?>">supprimer</a> </td>
    </tr>
    <?php } ?>
 </table>
