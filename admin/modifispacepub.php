...
<?php 
     require_once("connexio.php");
     if(!empty($_GET['code'])){
        $req= $bdd->prepare("SELECT * FROM space WHERE codes=?");
...
<h1> <a href="../index.php">ACCUEIL</a></h1>
   ...
   <div class="connexion1"> <a href="../index.php">ACCUEIL</a></div>
    <section class="tete">
        <div class="menu">
           <ul>
        ...
        <div class="formulaire1">
            <form action="modifispacepub.php" method="POST" enctype="multipart/form-data" class="formulaire2" >
               <label class="formulaire3" for="">CODE</label>
                <input type="text" name="logo" class="formule3" value="<?php echo ($modif['codes']) ?>">
                <label class="formulaire3" for="">TITRE</label> <br>
                <input type="text" name="logo1" class="formule3" value="<?php echo ($modif['titres']) ?>">
                <label class="formulaire3"  for="">DESCRIPTION</label> <br>
                <input type="text" name="logo2" id="" class="formule3" value="<?php echo ($modif['descriptions']) ?>">
                <label class="formulaire3"  for="">IMAGE</label> <br>
                <input type="file" name="logo3" class="formule3" >
                <img src="images/<?php echo ($modif['images']) ?>" alt="" width="60" height="60">
                <label class="formulaire3"  for="">TEXTE</label> <br>
                <textarea name="logo4" id="" class="formule3"> <?php echo ($modif['textes']) ?> </textarea>
...
<?php 
   require_once("connexio.php");
    ...
    if (isset($_POST['logo5'])) {
       ...
       if($photo1 === ""){
          $req= $bdd->prepare("UPDATE space SET titres=?, descriptions=?, textes=?  WHERE codes=?");
          $req->execute(array($_POST['logo1'],$_POST['logo2'],$_POST['logo4'],$_POST['logo']));
          header("location:affichespacepub.php");

        }
       
        else{
            $req= $bdd->prepare("UPDATE space SET titres=?, descriptions=?, images=?, textes=?  WHERE codes=?");
