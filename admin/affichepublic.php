...
<?php  
	if(!empty($_GET['code'])){	
		$supp=$bdd->prepare("DELETE FROM marchepublic WHERE code=?");    
...
<h1> <a href="../index.php">ACCUEIL</a></h1>
 ...
 <div class="formulai">
   ...
   <table>
	<h3 class="lien"> <a href="marchepublic.php">ajouter</a></h3>
	  <tr class="tab">
        <td>code</td>
        <td>TITRE</td>
        <td>DESCRIPTION</td>
	...
	<?php $reponse=$bdd->query("SELECT * FROM marchepublic"); ?>
  	<?php while ($donne=$reponse->fetch()) { ?>
    <tr>
        <td> <?php echo ($donne['code']) ?> </td>                          
        ...
        <td> <a href="modifimarchepublic.php?code=<?php echo ($donne['code'])?>">modifier</a> </td>
    	<td> <a href="affichepublic.php?code=<?php echo ($donne['code'])?>">supprimer</a> </td>
	</tr>            		
 	<?php } ?>
 </table>
