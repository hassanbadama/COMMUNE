...
<?php 
     ...
     require_once("connexio.php");
     if(!empty($_GET['code'])){
        $req= $bdd->prepare("SELECT * FROM lieutourist WHERE codt=?");
  ...
  <div class="connexion">  <?php echo "Bonjours ".$_SESSION['NOM']['nom'] ?>  </div>
    <section class="tete">
        ...
        <div class="formulaire1">
            <form action="modifilieutourist.php" method="POST" enctype="multipart/form-data" class="formulaire2" >
               <label class="formulaire3" for="">CODE</label>
                <input type="text" name="logo" class="formule3" value="<?php echo ($modif['codt']) ?>">
                <label class="formulaire3" for="">TITRE </label> <br>
                <input type="text" name="logo1" class="formule3" value="<?php echo ($modif['titre']) ?>">
                <label class="formulaire3"  for="">DESCRIPTION </label> <br>
                <input type="text" name="logo2" id="" class="formule3" value="<?php echo ($modif['descriptiont']) ?>">
                <label class="formulaire3"  for=""> IMAGE</label> <br>
                <input type="file" name="logo3" class="formule3" >
                <img src="images/<?php echo ($modif['imaget']) ?>" alt="" width="60" height="60">
                <label class="formulaire3"  for="">TEXTE </label> <br>
                <textarea name="logo4" id="" class="formule3"> <?php echo ($modif['texte']) ?> </textarea>
...
<?php 
   require_once("connexio.php");
    ...
    if (isset($_POST['logo5'])) {
       ...
       if($photo1 === ""){
          $req= $bdd->prepare("UPDATE lieutourist SET titre=?, descriptiont=?, texte=?  WHERE codt=?");
          $req->execute(array($_POST['logo1'],$_POST['logo2'],$_POST['logo4'],$_POST['logo']));
         header("location:affichelieutourist.php");

        }
       
        else{
            $req= $bdd->prepare("UPDATE lieutourist SET titre=?, descriptiont=?, imaget=?, texte=?  WHERE codt=?");
