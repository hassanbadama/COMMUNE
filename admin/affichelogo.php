<?php 
  ...
  if(!empty($_GET['code'])){
     	$supp=$bdd->prepare("DELETE FROM logo WHERE codl=?");    
...
<div class="connexion">  <?php echo "Bonjours ".$_SESSION['NOM']['nom'] ?>  </div>
 ...
 <div class="formulai">
  <table>
    <h3 class="lien"> <a href="logo.php">ajouter</a></h3>
    <tr class="tab">
         <td>code</td>
        <td>TITRE</td>
         <td>DESCRIPTION</td>
	...
	<?php $reponse=$bdd->query("SELECT * FROM logo"); ?>
	<?php while ($donne=$reponse->fetch()) { ?>
    <tr>
    	<td> <?php echo ($donne['codl']) ?> </td> 
        <td> <?php echo ($donne['titrel']) ?> </td>
        <td> <?php echo ($donne['descriptionl']) ?> </td>
        <td> <img src="images/<?php echo ($donne['logo']) ?>" alt="" width="60" height="60">  </td>
         <td> <img src="images/<?php echo ($donne['imagefont']) ?> " alt="" width="60" height="60"> </td>
        <td> <a href="modifilogo.php?code=<?php echo ($donne['codl'])?>">modifier</a> </td>
        <td> <a href="affichelogo.php?code=<?php echo ($donne['codl'])?>">supprimer</a> </td>
	</tr>
   	<?php 
} ?>
 </table>
