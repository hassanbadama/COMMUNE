...
<?php 
     require_once("connexio.php");
     if(!empty($_GET['code'])){
        $req= $bdd->prepare("SELECT * FROM marchepublic WHERE code=?");
        ...
        <div class="formulaire1">
            <form action="" method="POST" enctype="multipart/form-data" class="formulaire2" >
               <label class="formulaire3" for="">CODE </label>
                <input type="text" name="logo" class="formule3" value="<?php echo ($modif['code']) ?>">
                ...
                <textarea name="logo3" id="" class="formule3"><?php echo ($modif['texte']) ?></textarea>
...
<?php 
   require_once("connexio.php");
    ...
    if (isset($_POST['logo5'])) {
       if($_FILES['pho']['name']===""){
            $req= $bdd->prepare("UPDATE marchepublic SET titre=?, description=?, texte=? WHERE code=?");
            $req->execute(array($_POST['logo1'],$_POST['logo2'],$_POST['logo3'],$_POST['logo']));
            header("location:affichepublic.php");
       }
        else{
            $photo1 = $_FILES['pho']['name'];
            $fichier1 = $_FILES['pho']['tmp_name'];
            move_uploaded_file($fichier1,'./images/'.$photo1);
            $req= $bdd->prepare("UPDATE marchepublic SET titre=?, description=?, texte=?, image=? WHERE code=?");
